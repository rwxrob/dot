...
nnoremap coming i_Coming soon..._<Esc>

set ruf=%30(%=%#LineNr#%.50F\ [%{strlen(&ft)?&ft:'none'}]\ %l:%c\ %p%%%)

if has('nvim')
:lua << EOF
-- Customize the sign for errors
vim.fn.sign_define('DiagnosticSignError', { text = '🔥', texthl = 'DiagnosticError' })
-- Customize the sign for warnings
vim.fn.sign_define('DiagnosticSignWarn', { text = '❗️', texthl = 'DiagnosticWarn' })
-- Customize the sign for information
vim.fn.sign_define('DiagnosticSignInfo', { text = '✨', texthl = 'DiagnosticInfo' })
-- Customize the sign for hints
...
if filereadable(expand("~/.vim/autoload/plug.vim"))
  ...
  if has('nvim') && exists('$NVIM_SCREENKEY')
    Plug 'neoclide/coc.nvim', {'branch': 'release'}
    let g:screenkey_statusline_component=1
    Plug 'NStefan002/screenkey.nvim'
    set winbar=%{%v:lua.require('screenkey').get_keys()%}
    nnoremap sc :Screenkey<CR>
  endif
  call plug#end()

  if has('nvim') && exists('$NVIM_SCREENKEY')
    :lua <<EOF
require('screenkey').setup({win_opts={width=90}})
EOF
  endif
  ...
  " gruvbox-material (colorscheme MUST come last)
